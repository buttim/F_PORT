        ...
        if data[0] == 'FRAME':
            ...
            if not self.uplink and not self.inframe and self.type == 1:
                ...
                if self.type == 1 and ss < self.lastframees + 3000:
                    ...
                    self.appid = 0
                    self.data = 0
                    self.put(ss, es, self.out_ann, [1, [format(byte, 'X')]])
                    return
            ...
            if self.uplink:
                ...
                if self.nbyte == 0:       # type
                    self.type = byte
                    self.put(ss, es, self.out_ann, [3, [format(byte, 'X')]])
                elif self.nbyte == 1:     # prim
                    self.prim = byte
                    self.put(ss, es, self.out_ann, [9, [format(byte, 'X')]])
                    self.ss = ss
